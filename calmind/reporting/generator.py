...
class ReportGenerator:
    def __init__(self, reports_dir="reports", templates_dir="templates"):
        ...
        print(f"ReportGenerator: Initializing with reports_dir={self.reports_dir}, templates_dir={self.templates_dir}")
        os.makedirs(self.reports_dir, exist_ok=True)

    def _load_template(self, template_name):
        template_path = os.path.join(self.templates_dir, template_name)
        print(f"ReportGenerator: Attempting to load template from {template_path}")
        try:
            with open(template_path, 'r', encoding='utf-8') as f:
                return f.read()
        except FileNotFoundError:
            print(f"ReportGenerator Error: Template file not found at {template_path}")
            return "<h1>Template Not Found!</h1><p>{content}</p>"

    def generate_html_report(self, user_name: str, summary_content: str) -> str:
        print(f"ReportGenerator: Generating HTML report for {user_name}...")
        ...
        print(f"ReportGenerator: HTML report generated: {file_path}")
        return file_path

    def generate_md_report(self, user_name: str, summary_content: str) -> str:
        print(f"ReportGenerator: Generating Markdown report for {user_name}...")
        report_date = datetime.now().strftime('%Y-%m-%d %H:%M')
        ...
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(md_report)
        print(f"ReportGenerator: Markdown report generated: {file_path}")
        return file_path
